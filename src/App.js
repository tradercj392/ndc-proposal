    ...
    if (mode === "preview") {
      ...
      <div class='opt ${selectedOption === "standard" ? "sel" : ""}' onclick="window.parent.postMessage({type:'selectOption',option:'standard'},'*')">
        <div style='display:flex;justify-content:space-between;align-items:center'>
          ...
          <div style='text-align:right'>
            ...
            ${standardMonthly ? "<div style='font-size:18px;font-weight:800;color:#0f172a;margin-top:4px'>$" + standardMonthly.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + "<span style='font-size:13px;color:#64748b;font-weight:600'>/mo</span></div>" : ""}
          </div>
        </div>
      ...
      <div class='opt ${selectedOption === "priority" ? "sel" : ""}' onclick="window.parent.postMessage({type:'selectOption',option:'priority'},'*')">
        <div style='display:flex;justify-content:space-between;align-items:center'>
          ...
          ${selectedOption === "priority" ? "<div style='text-align:right'><div style='font-size:24px;font-weight:800;color:#0ea5e9'>" + fmt(priority) + "</div>" + (monthlyPayment ? "<div style='font-size:18px;font-weight:800;color:#0f172a;margin-top:4px'>$" + monthlyPayment.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + "<span style='font-size:13px;color:#64748b;font-weight:600'>/mo</span></div>" : "") + "</div>" : ""}
        </div>
        ${selectedOption === "priority" ? "<div style='font-size:20px;font-weight:800;color:#166534;margin-top:8px'>You save " + fmt(standard - priority) + "</div>" + adminItemizedBlock : ""}
      </div>
      ...
      // pdf — static — show only selected option's pricing
    ...
    <div style={{ padding: "0 0 24px" }}>
      ...
      <div style={{ padding: "16px 24px 0" }}>
        ...
        {pricingRevealed && selectedOption && (
          <div style={{ background: selectedOption === "clearance" ? "#fffbeb" : "#f0f9ff", border: "1.5px solid " + (selectedOption === "clearance" ? "#fde68a" : "#bae6fd"), borderRadius: 8, padding: "10px 14px", marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ marginBottom: 16, paddingBottom: 16, borderBottom: "1px solid #f1f5f9" }}>
              ...
              {applicableMonthly && (
                <div style={{ marginTop: 8 }}>
                  <div onClick={() => setUsingFinancing(f => !f)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", border: "1.5px solid " + (usingFinancing ? "#0ea5e9" : "#e2e8f0"), borderRadius: 8, background: usingFinancing ? "#f0f9ff" : "#f8fafc", cursor: "pointer", marginBottom: usingFinancing ? 12 : 0 }}>
                    ...
                    <div style={{ background: "#f0f9ff", border: "1.5px solid #bae6fd", borderRadius: 10, padding: 16 }}>
                      <div style={{ marginBottom: 14 }}>
                        ...
                        <div style={{ background: "white", borderRadius: 8, border: "1px solid #bae6fd", overflow: "hidden" }}>
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 12px", borderBottom: "1px solid #f0f9ff" }}><span style={{ fontSize: 11, color: "#475569" }}>Due out of pocket ({100 - financingPct}%)</span><span style={{ fontSize: 12, fontWeight: 700, color: "#0f172a" }}>{fmt(chosenTotal * (100 - financingPct) / 100)}</span></div>
                          <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 12px" }}><span style={{ fontSize: 11, color: "#475569" }}>Approx. monthly payment</span><span style={{ fontSize: 13, fontWeight: 800, color: "#0f172a" }}>${(applicableMonthly * financingPct / 100).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}/mo</span></div>
                        </div>
                      </div>
              ...
              <div style={{ marginTop: 12 }}>
                <div onClick={() => setShowDeposit(d => !d)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", border: "1.5px solid " + (showDeposit ? "#0ea5e9" : "#e2e8f0"), borderRadius: 8, background: showDeposit ? "#f0f9ff" : "#f8fafc", cursor: "pointer", marginBottom: showDeposit ? 12 : 0 }}>
                ...
                {showDeposit && (
                  <div style={{ border: "1.5px solid #bae6fd", borderRadius: 10, overflow: "hidden", marginBottom: 10 }}>
                    {[
                      { key: "50", label: "50% Deposit", lines: [fmt(chosenTotal * 0.5) + " due at signing (50%)", fmt(chosenTotal * 0.5) + " due upon completion (50%)"] },
                      { key: "33", label: "33% / 33% / 33% — Three Payments", lines: [fmt(chosenTotal * 0.33) + " due at signing (33%)", fmt(chosenTotal * 0.33) + " due when materials are delivered and crew starts (33%)", fmt(chosenTotal - chosenTotal * 0.33 * 2) + " due upon completion (remaining balance)"] },
                    ].map((opt, i) => (
                      <div key={opt.key} onClick={() => setDepositOption(opt.key)} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "14px 16px", borderBottom: "1px solid #e2e8f0", background: depositOption === opt.key ? "#f0f9ff" : "white", cursor: "pointer" }}>
                        <div style={{ width: 18, height: 18, borderRadius: "50%", border: "2px solid " + (depositOption === opt.key ? "#0ea5e9" : "#cbd5e1"), background: depositOption === opt.key ? "#0ea5e9" : "white", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
                          {depositOption === opt.key && <div style={{ width: 6, height: 6, borderRadius: "50%", background: "white" }} />}
                        </div>
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 700, color: "#0f172a", marginBottom: 4 }}>{opt.label}</div>
                          {opt.lines.map((l, j) => <div key={j} style={{ fontSize: 11, color: "#475569", lineHeight: 1.7 }}>• {l}</div>)}
                        </div>
                      </div>
                    ))}
                    <div onClick={() => setDepositOption("custom")} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "14px 16px", background: depositOption === "custom" ? "#f0f9ff" : "white", cursor: "pointer" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {depositOption === "custom"
                          ? <textarea value={customDepositText} onChange={e => setCustomDepositText(e.target.value)} onClick={e => e.stopPropagation()} placeholder="e.g. $5,000 due at signing, remainder financed through Aqua..." style={{ width: "100%", boxSizing: "border-box", border: "1.5px solid #bae6fd", borderRadius: 8, padding: "10px 12px", fontSize: 12, color: "#0f172a", outline: "none", height: 80, resize: "vertical", lineHeight: 1.6 }} />
                ...
                {showDeposit && depositOption && (
                  <div style={{ background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: 8, padding: "12px 14px", fontSize: 11, color: "#0f172a", lineHeight: 1.8 }}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (newWin) {
                    ...
                    newWin.document.title = "NDC_Proposal_" + clientName + "_" + dateStr;
